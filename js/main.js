...
/* Google font from Generator */
var teaserFontFamily = "TeXGyreHeros";
var googleFontArray = ['Poppins:300,400,500,600'];

$(function() {
	if ($('body').attr('data-stomp-google-font')) {
		googleFontArray.push($('body').attr('data-stomp-google-font'));
		teaserFontFamily = $('body').attr('data-stomp-family-name');
	}
	/* ==================================================================
	1.0 Load Custom, Google Fonts
	================================================================== */
	WebFont.load({
		...
		google: {
			families: googleFontArray
		},
		classes: false,
		timeout: 5000,
		fontactive: function(familyName, fvd) {
			if(familyName == teaserFontFamily) {
	...
	// Change Secondary BG and tinge (used in scenes 14,33)
	primaryTinge = $svenContainer.attr("data-primary-tinge");
	secondaryBG = $svenContainer.attr("data-secondary-bg");
	secondaryTinge = $svenContainer.attr("data-secondary-tinge");

	var freezeProp = $svenContainer.attr("data-freeze-prop") ? parseBool($svenContainer.attr("data-freeze-prop")) : true; // False only for Video Convesion
	var letterBoxingProp = $svenContainer.attr("data-letterboxing") == "yes" ? true : false;
	...
	$('.pl-skip').on('click' , function(ev) {
		...
		if($('.pl-sound').parent().hasClass("highlight")) { // Fixed audio bug
